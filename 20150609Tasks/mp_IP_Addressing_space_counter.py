...
# 优化手段4：多线程优化(基本的process=>start=>join), 对5各VP一起执行的速度由180多秒 减少至 60 second 左右，还有待继续优化
# 优化手段5：多线程优化之优化，比如使用进程池
...
def worker(vp, given_date_list):
    """
        Take a vantage point name and a list of date time in which we are interested as inout, and print the executing
        result into the terminal. This worker function will be placed into a process.
    """
        ...
        for line in f_handler:
            ...
            if tmp_list[LOG_TIME_COLUMN['round_type_set']] != 'RoundNoReply':
                ...
                for key, value in tmp_dict.iteritems():
                   prefix_size_dic[key].extend(value)
    for key, value in prefix_size_dic.iteritems():
        prefix_size_dic[key] = prefix_size(merge_all_prefixes(list(set(value))))
        print "In", vp, "and in date:", key, "the prefix size is:", prefix_size_dic[key]
...
if __name__ == '__main__':
    ...
    # manager = Manager()
    #
    # prefix_size_dic = manager.dict()
    #
    # for vp in VP_LIST:
    #     prefix_size_dic[vp] = manager.dict()
    ...
    for i in range(volume):
        pools.append(Process(target=worker, args=(VP_LIST[i], given_date_list,)))
    ...
    # for key in prefix_size_dic.keys():
    #     print key, prefix_size_dic[key], type(prefix_size_dic[key])
    #     for given_date in prefix_size_dic[key].keys():
    #         print given_date, "xxxx"
