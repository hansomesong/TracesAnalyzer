...
if __name__ == '__main__':
    ...
    rects2 = plt.bar(index, consistent_number, bar_width, color='blue',label= 'correlated groups')

    plt.xlabel('vantage point', fontsize=22)
    # plt.ylabel('number', fontsize=22)
    # plt.title('Number of RLOC associated with different prefixes')
    plt.xticks(index + 0.5*bar_width, index + 1)
    plt.ylim(0,28)
    plt.legend()
    plt.savefig(
            os.path.join(PLOT_DIR, 'Number_RLOC_different_prefixes.eps'),
            dpi=300
        )
